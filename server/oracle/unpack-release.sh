...
usage() {
  ...
  echo "Example: $0" >&2
  echo "Example: $0 oracle-linux-amd64.tar.gz $SCRIPT_DIR" >&2
}
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/oracle-release.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
...
for name in oracle start.sh stop.sh; do
  if [ -f "$RELEASE_DIR/$name" ]; then
    chmod +x "$RELEASE_DIR/$name"
  fi
done

if [ -d "$RELEASE_DIR/script" ]; then
  for f in "$RELEASE_DIR/script/"*.sh; do
    [ -f "$f" ] && chmod +x "$f"
  done
fi
