...
// Analyze 用快照 + 特征(+可选消息面)调用 LLM，返回结构化预测与原始返回。
// newsSummary 为空时不注入消息面；消息面仅作辅助修正项，技术面仍为主导。
func (a *Analyzer) Analyze(ctx context.Context, snap *collector.Snapshot, f indicator.Features, newsSummary string) (*Decision, string, error) {
	...
	finalizeDecision(&decision, f, pricePrecision(snap))
...
func finalizeDecision(d *Decision, f indicator.Features, pricePrec int) {
...
func ToSaveDTO(snap *collector.Snapshot, f indicator.Features, d *Decision, provider, model, newsSummary string) tradeDTO.TradeAIPredictionSaveDTO {
	predictTime := time.Now().Add(intervalDuration(snap.Interval)).Unix()
