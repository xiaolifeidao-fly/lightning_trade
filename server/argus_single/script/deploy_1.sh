...
# Argus_single 项目部署脚本
# 从环境变量获取配置
remote_server="root@${argus_single_remote_server:-}"
remote_password="${argus_single_password:-}"
...
if [ -z "$remote_server" ] || [ -z "$remote_password" ]; then
    echo "❌ 错误: 请设置环境变量:"
    echo "   export argus_single_remote_server=your_server_ip"
    echo "   export argus_single_password=your_password"
    echo "   export argus_single_port=22  # 可选，默认22"
    exit 1
fi
