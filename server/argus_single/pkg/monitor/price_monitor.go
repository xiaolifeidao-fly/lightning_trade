...
type SymbolConfig struct {
	...
	SignalThreshold float64 // 盘口信号阈值（LastPrice 与 MarkedPrice 偏离比例）
}
...
type PriceMonitor struct {
	...
	lastTradeTime      map[string]time.Time       // 上次交易时间（防重复）
	tradeMu            sync.Mutex                 // 交易时间锁
	signalScheduler    *SignalScheduler           // 盘口信号延迟调度器
...
func NewPriceMonitor(symbolConfigs map[string]SymbolConfig) *PriceMonitor {
	...
	return &PriceMonitor{
		...
		binancePriceTimes:  make(map[string]time.Time),
		deepcoinPriceTimes: make(map[string]time.Time),
...
func (pm *PriceMonitor) Start() {
	...
	for symbol := range pm.symbolConfigs {
		...
		pm.binancePriceTimes[symbol] = time.Time{}
		pm.deepcoinPriceTimes[symbol] = time.Time{}
...
func (pm *PriceMonitor) subscribeBinance(symbol string) {
	...
	// USDⓈ-M futures market stream. Do not use the spot stream here; DeepCoin
	// is monitored as a SWAP contract, so Binance must be a futures quote too.
...
func (pm *PriceMonitor) handleBinanceMessage(symbol string, message []byte) {
	...
	pm.binancePriceTimes[symbol] = time.Now()
	deepPrice := pm.deepcoinPrices[symbol]
	deepTime := pm.deepcoinPriceTimes[symbol]
...
func (pm *PriceMonitor) handleDeepCoinMessageNew(symbol string, message []byte) {
	...
	pm.handleOrderBookSignal(symbol, dataField, price)

	pm.mu.Lock()
	pm.deepcoinPrices[symbol] = price
	pm.deepcoinPriceTimes[symbol] = time.Now()
	...
	// 检查价差（在goroutine外检查，避免重复发送）
	if binPrice > 0 {
		pm.checkPriceDiff(symbol, binPrice, price, binTime, deepTime)
	}
}
...
func normalizeSignalText(value string) (SignalDirection, bool) {
	normalized := strings.ToUpper(strings.TrimSpace(value))
	switch {
	case normalized == "UP" || normalized == "LONG" || normalized == "BUY" || strings.Contains(value, "上涨"):
		return SignalDirectionUp, true
	case normalized == "DOWN" || normalized == "SHORT" || normalized == "SELL" || strings.Contains(value, "下跌"):
...
func (pm *PriceMonitor) handleOrderBookSignal(symbol string, dataField map[string]interface{}, fallbackPrice float64) {
	if !hasSignalTradeAccounts() || pm.signalScheduler == nil {
		return
	}
	...
	if explicitSignal, ok := getStringFromMap(dataField, "Signal", "signal", "Direction", "direction", "Trend", "trend"); ok {
		if direction, ok := normalizeSignalText(explicitSignal); ok {
			price := fallbackPrice
			if lastPrice, found := getFloatFromMap(dataField, "LastPrice", "last", "price", "N"); found {
				price = lastPrice
			}
			logrus.Infof("[%s] 收到显式盘口信号: %s, price=%.8f", symbol, direction, price)
			pm.signalScheduler.OnSignal(symbol, config, direction, price, "explicit", trade.SignalQuote{})
		}
		return
	}

	lastPrice, hasLastPrice := getFloatFromMap(dataField, "LastPrice", "last", "price", "N")
	markedPrice, hasMarkedPrice := getFloatFromMap(dataField, "MarkedPrice", "markedPrice", "MarkPrice", "markPrice", "M")
	if !hasLastPrice || !hasMarkedPrice || markedPrice == 0 {
		return
	}
...
func getSpreadMaxPriceAge() time.Duration {
	maxAgeMs := vipper.GetInt("monitor.spread.max_price_age_ms")
	if maxAgeMs <= 0 {
		maxAgeMs = 500
	}
	return time.Duration(maxAgeMs) * time.Millisecond
}

func isSpreadThresholdCross(prevSpread float64, hasPrevSpread bool, signedSpread, threshold float64) bool {
	if !hasPrevSpread {
		return false
	}
	return (prevSpread < threshold && signedSpread >= threshold) ||
		(prevSpread > -threshold && signedSpread <= -threshold)
}

// checkPriceDiff 检查价差并发送通知。
// signedSpread > 0 表示 Binance > DeepCoin，signedSpread < 0 表示 DeepCoin > Binance。
// 只在有方向价差从阈值内穿越到阈值外时触发，避免长期固定基差反复开仓。
func (pm *PriceMonitor) checkPriceDiff(symbol string, binPrice, deepPrice float64, binTime, deepTime time.Time) {
	...
	if !hasSpreadTradeAccounts() {
		return
	}

	maxAge := getSpreadMaxPriceAge()
	now := time.Now()
	if binTime.IsZero() || deepTime.IsZero() {
		logrus.Debugf("%s 跳过价差检查: 行情时间未初始化, BinanceTime=%v, DeepCoinTime=%v", symbol, binTime, deepTime)
		return
	}
	binAge := now.Sub(binTime)
	deepAge := now.Sub(deepTime)
	if binAge > maxAge || deepAge > maxAge {
		logrus.Debugf("%s 跳过价差检查: 行情不同步, BinanceAge=%s, DeepCoinAge=%s, maxAge=%s",
			symbol, binAge, deepAge, maxAge)
		return
	}

	// 计算有方向价差百分比
	signedSpread := (binPrice - deepPrice) / deepPrice
	diffPercent := math.Abs(signedSpread)

	pm.mu.Lock()
	prevSpread, hasPrevSpread := pm.lastSpread[symbol]
	pm.lastSpread[symbol] = signedSpread
	pm.mu.Unlock()

	if !hasPrevSpread {
		logrus.Debugf("%s 初始化价差基线: signedSpread=%.4f%%, 阈值: %.4f%%, Binance: %.8f, DeepCoin: %.8f",
			symbol, signedSpread*100, config.Threshold*100, binPrice, deepPrice)
		return
	}

	triggered := isSpreadThresholdCross(prevSpread, hasPrevSpread, signedSpread, config.Threshold)

	// 如果价差从阈值内穿越到阈值外
	if triggered {
		// 记录日志
		timestamp := utils.GetCurrentTimeString()
		logrus.Infof("[%s] %s 价差穿越阈值: signed=%.4f%%, abs=%.4f%%, prev=%.4f%%, 阈值: %.4f%%, Binance: %.8f, DeepCoin: %.8f",
			timestamp, symbol, signedSpread*100, diffPercent*100, prevSpread*100, config.Threshold*100, binPrice, deepPrice)
		...
		timestamp := utils.GetCurrentTimeString()
		logrus.Debugf("[%s] %s 价差未穿越: signed=%.4f%%, abs=%.4f%%, prev=%.4f%%, 阈值: %.4f%%, Binance: %.8f, DeepCoin: %.8f",
			timestamp, symbol, signedSpread*100, diffPercent*100, prevSpread*100, config.Threshold*100, binPrice, deepPrice)
	}
}
...
func (pm *PriceMonitor) sendTelegramAlert(symbol string, binPrice, deepPrice, diffPercent, threshold float64) {
	...
	pm.alertMu.Lock()
	lastTime, exists := pm.lastAlertTime[symbol]
	now := time.Now()

	if exists && now.Sub(lastTime) < 1*time.Second {
		pm.alertMu.Unlock()
		logrus.Debugf("%s 告警过于频繁，跳过发送 (距上次: %.1fs)", symbol, now.Sub(lastTime).Seconds())
		return
	}

	// 2. 通过检查，立即更新时间并释放锁（这样后续的goroutine会被拦截）
	pm.lastAlertTime[symbol] = now
	pm.alertMu.Unlock()
...
func (pm *PriceMonitor) sendInsufficientBalanceAlert(symbol string, binPrice, deepPrice, diffPercent, threshold float64, insufficientAccounts []string) {
	...
	if exists && now.Sub(lastTime) < 1*time.Second {
		pm.alertMu.Unlock()
		logrus.Debugf("%s 余额不足告警过于频繁，跳过发送 (距上次: %.1fs)", symbol, now.Sub(lastTime).Seconds())
		return
	}
...
func (pm *PriceMonitor) executeArbitrageTrade(symbol, instId string, binPrice, deepPrice float64) {
	...
	pm.tradeMu.Lock()
	lastTime, exists := pm.lastTradeTime[symbol]
	now := time.Now()

	if exists && now.Sub(lastTime) < 2*time.Second {
		pm.tradeMu.Unlock()
		logrus.Debugf("%s 套利交易过于频繁，跳过执行 (距上次: %.1fs)", symbol, now.Sub(lastTime).Seconds())
		return
	}

	// 2. 通过检查，立即更新时间并释放锁（这样后续的goroutine会被拦截）
	pm.lastTradeTime[symbol] = now
	pm.tradeMu.Unlock()

	// 3. 执行交易（不持有锁，不影响其他检查）
	logrus.Infof("🎯 %s 触发套利交易: 币安=%.2f, DeepCoin=%.2f, 合约=%s",
		symbol, binPrice, deepPrice, instId)
